...
function parseCommandLineArgs() {
  const argv = yargs
    ...
    .example(
      '-s stringarray.txt -w stringarraywrappers.txt -n _0x471d,_0x9cad,_0x4cd9 ' +
    ...
    .option('debug', {
      alias: 'd',
      description:
        'Display debug log messages (for large input files, debug log might be large).',
      type: 'boolean',
      default: false,
    })
    .option('debugall', {
      alias: 'a',
      description:
        "Display all AST operations for debugging. The 'debug' option needs to be set " +
        'for this to work.',
      type: 'boolean',
      default: false,
    })
    .option('separateloggers', {
      alias: 'l',
      description:
        'Use a separate logger for each deobfuscation stage. This will lead to the creation of a ' +
    ...
    .option('writedifffiles', {
      alias: 'f',
      description:
        'Write before&after files for each transformation modifying the source code' +
    ...
    .option('stringarray', {
      alias: 's',
      description:
        '[StringArray] The path at which the code for the string array is found' +
    ...
    .option('stringarraywrappers', {
      alias: 'w',
      description:
        '[StringArray] The path at which the code for the string wrappers is found' +
    ...
    .option('stringarraywrappernames', {
      alias: 'n',
      description:
        '[StringArray] The names of the string array wrapper functions in a comma separated string ' +
    ...
    .option('stringarrayrotate', {
      alias: 'r',
      desscription:
        '[StringArray] The path at which the code for the string array rotate function ' +
    ...
    .option('cleanuplog', {
      alias: 'c',
      desscription:
        'The log files are created by appending to existing ones. This option deletes the existing ' +
...
function _validateStringArrayCommandLineArgs(argv) {
  if (
    ...
    if (
      ...
      console.log(
        "Invalid command line usage. If any of the '[StringArray]' non-optional command line " +
...
function _extractStringArraySpecificCmdLineNodesInfo(argv) {
  ...
  argv.stringArrayCmdLineSpecificNodesInfo = {
    stringArrayTemplateCode: _readCodeForStringArrayCmdLineSpecificInfo(
      argv.stringarray,
    ),
    stringArrayWrapperTemplates: _readCodeForStringArrayCmdLineSpecificInfo(
      argv.stringarraywrappers,
    ),
...
function main() {
  ...
  logger.info(
    `[index.js] Starting deobfuscation process for file ${argv.inputPath}.`,
  );
  ...
  try {
    ...
    logger.error(
      `[index.js] Deobfuscation error in main function. error = ${e}. Stack = \n${e.stack}`,
    );
  }

  usefulModule.writeTextFile(argv.outputPath, deobfuscatedSourceCode);
  logger.debug(
    `[index.js] Deobfuscated source code: \n${deobfuscatedSourceCode}`,
  );
