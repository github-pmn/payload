...
class DeadCodeInjectionDeadCodeInjectionTransformer extends stageDeobfuscator.TransformerDeobfuscator {
      ...
      estraverse.traverse(astNodes[i], {
        enter: function (node) {
          ...
          if (thisObj.nrBlockStatements % 1000 == 0) {
            console.log(
              `[stage_08][DeadCodeInjectionDeadCodeInjectionTransformer] Deobfuscate nrBlockStatements = ${thisObj.nrBlockStatements}.`,
            );
          }
  ...
  _deobfuscateBlockStatementWithDeadCodeInjected(node) {
    ...
    for (let i = 0; i < node.body.length; i++) {
      ...
      if (
        ...
        ifStatement.consequent.body.length == 0 ||
        ifStatement.alternate.body.length == 0
      ) {
        continue;
      }

      if (
        !this._isTestConditionFoundInIfStatementOfInjectedDeadCode(
          ifStatement.test,
        )
      ) {
        continue;
      ...
      let bodyWithoutDeadCode = null;
      let bodyWithDeadCode = null;
      let partRemoved = '';
  ...
  _removeDeadCodeDanglingDictionaries() {
    console.log(
      `[stage_08][DeadCodeInjectionDeadCodeInjectionTransformer] Dangling remove-START.`,
    );
    ...
    for (let i = 0; i < objectExpressionNodes.length; i++) {
      ...
      if (nrObjExprNodes == 1 && nrObjExprNodes % 100 == 0) {
        console.log(
          `[stage_08][DeadCodeInjectionDeadCodeInjectionTransformer] Dangling remove ` +
      ...
      for (let j = 0; j < objectExpression.properties.length; j++) {
        const property = objectExpression.properties[j];
        ...
        const identifiersWithProperty = esquery(
