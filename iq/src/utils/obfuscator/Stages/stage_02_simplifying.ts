...
class StageSimplifying extends stageDeobfuscator.StageDeobfuscator {
  ...
  _initiateTransformers() {
    this.transformers = {
      SimplifyingIfStatementSimplifyTransformer:
        SimplifyingIfStatementSimplifyTransformer,
      SimplifyingBlockStatementSimplifyTransformer:
        SimplifyingBlockStatementSimplifyTransformer,
      SimplifyingExpressionStatementsMergeTransformer:
        SimplifyingExpressionStatementsMergeTransformer,
      SimplifyingCustomCodeHelpersTransformer:
        SimplifyingCustomCodeHelpersTransformer,
      SimplifyingVariableDeclarationsMergeTransformer:
        SimplifyingVariableDeclarationsMergeTransformer,
    };
  }
  ...
  deobfuscate() {
    ...
    do {
      ...
      if (this.sourceCodeBeforeStage != sourceCodeAfterStage) {
        ...
        this.logger.info(
          `[stage_02_simplifying.js] Found difference in simplifying. Running again.`,
        );
      }
...
class SimplifyingIfStatementSimplifyTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  _expandIfStatementConsequentWithBlockStatement(node) {
    astOperations.ASTModifier.logDebugNode(
      '[Simplify][IfStatement][Adding BlockStatement][Consequent][Old]',
      node.consequent,
      this.logger,
      this.obfuscatedSourceCode,
    );
    node.consequent = this._createConsequentOrAlternateWithBlockStatement(
      node.consequent,
    );
    astOperations.ASTModifier.logDebugNode(
      '[Simplify][IfStatement][Adding BlockStatement][Consequent][New]',
      node.consequent,
      this.logger,
      this.obfuscatedSourceCode,
    );
  ...
  _expandIfStatementAlternateWithBlockStatement(node) {
    astOperations.ASTModifier.logDebugNode(
      '[Simplify][IfStatement][Adding BlockStatement][Alternate][Old]',
      node.alternate,
      this.logger,
      this.obfuscatedSourceCode,
    );
    node.alternate = this._createConsequentOrAlternateWithBlockStatement(
      node.alternate,
    );
    astOperations.ASTModifier.logDebugNode(
      '[Simplify][IfStatement][Adding BlockStatement][Alternate][New]',
      node.alternate,
      this.logger,
      this.obfuscatedSourceCode,
    );
...
class SimplifyingBlockStatementSimplifyTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  _expandMergedReturnAndExpressionStatements(node) {
    ...
    const sequenceExpression = node.argument;
    if (
    ...
    astOperations.ASTModifier.insertNodesBefore(
      node,
      nodesToAdd,
      this.logger,
      this.obfuscatedSourceCode,
  ...
  _removeExtraNodesFromReturnStatement(returnStatement, expressions) {
    astOperations.ASTModifier.logDebugNode(
      '[Simplify][ReturnStatement][Removing sequence expression ' +
        'from return statement ][Old]',
      returnStatement,
      this.logger,
      this.obfuscatedSourceCode,
    );
...
class SimplifyingExpressionStatementsMergeTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  _expandMergedExpressionStatements(node) {
    ...
    const expressionsArray = expression.expressions;
...
class SimplifyingVariableDeclarationsMergeTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  _removeExtraVariableDeclaratorsFromVariableDeclaration(node) {
    astOperations.ASTModifier.logDebugNode(
      '[Simplify][VariableDeclaration][Removing extra VariableDeclarators ' +
        'from return VariabeDeclaration][Old]',
      node,
      this.logger,
      this.obfuscatedSourceCode,
    );
    ...
    astOperations.ASTModifier.logDebugNode(
      '[Simplify][VariableDeclaration][Removing extra VariableDeclarators ' +
        'from return VariabeDeclaration][New]',
      node,
      this.logger,
      this.obfuscatedSourceCode,
      true,
