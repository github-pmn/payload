...
class StringArrayTemplateNodeFinder extends NodeFinder {
  ...
  _searchForFunctionDeclarationTemplate() {
    const possibleArrayTemplateNodes =
      this._findPossibleFunctionDeclarationTemplateNodes();
    possibleArrayTemplateNodes.forEach((functionDeclarationNode) => {
      if (
        this._functionDeclarationisStringArrayTemplate(functionDeclarationNode)
      ) {
  ...
  _functionDeclarationisStringArrayTemplate(node) {
    ...
    for (let i = 0; i < arrayExpressionNode.elements.length; i++) {
      if (
        arrayExpressionNode.elements[i].type != 'Literal' ||
    ...
    if (
      expressionStatementNode.expression.type != 'AssignmentExpression' ||
    ...
    if (
      functionExpressionNode.body.type != 'BlockStatement' ||
    ...
    const returnStatementOfInternalFunctionNode =
      functionExpressionNode.body.body[0];
    if (
      returnStatementOfInternalFunctionNode.argument.type != 'Identifier' ||
    ...
    if (
      returnStatementNode.argument.type != 'CallExpression' ||
    ...
    if (calleeNode.type != 'Identifier' || calleeNode.name != functionName) {
      return false;
    }
  ...
  _findIdentifierName() {
    ...
    if (
      this.stringArrayTemplateNode.type != 'FunctionDeclaration' &&
...
class StringArrayWrapperTemplateNodesFinder extends NodeFinder {
  constructor(
    ...
    ast,
    stringArrayTemplateIdentifierName,
  ) {
    super(logger, obfuscatedSourceCode, ast);
  ...
  _findNodesInAST() {
    ...
    const possibleWrapperNodes = this._findPossibleArrayWrapperNodes();
  ...
  _findPossibleArrayWrapperNodes() {
    ...
    const returnStatementNodes = esquery(
      this.ast,
      `Program > FunctionDeclaration > BlockStatement  VariableDeclaration ~ ` +
        `ReturnStatement`,
    );
    returnStatementNodes.forEach((node) => {
      const functionDeclarationNode =
        astOperations.ASTRelations.getParentNodeOfType(
          node,
          'FunctionDeclaration',
        );
  ...
  _isStringArrayWrapperTemplateFunction(node) {
    ...
    let foundStringArraytemplateIdentifier = false;
    const thisObj = this;
    estraverse.traverse(blockStatement.body[0], {
      enter: function (node) {
        ...
        if (
          thisObj._variableDeclarationContainsStringArrayTemplateIdentifier(
            node,
          )
        ) {
    ...
    const assignmentExpressions = esquery(
      node,
      `FunctionExpression[params.length=2] > BlockStatement >` +
    ...
    const callExpressionsAtTheEnd = esquery(
      node,
  ...
  _variableDeclarationContainsStringArrayTemplateIdentifier(node) {
    ...
    if (
      ...
      variableDeclaratorNode.id.type != 'Identifier'
    ) {
      return false;
    }
    ...
    if (
      variableDeclaratorNode.init.type != 'CallExpression' &&
      variableDeclaratorNode.init.type != 'MemberExpression'
    ) {
      return false;
    }
    ...
    if (variableDeclaratorNode.init.type == 'CallExpression') {
      ...
      if (
        !this._callExpressionContainsStringArrayTemplateIdentifier(
          callExpressionNode,
        )
      ) {
        return false;
      ...
      if (
        !this._memberExpressionContainsStringArrayTemplateIdentifier(
  ...
  _findArrayWrapperTemplatesForOlderObfuscatorVersions() {
    const memberExpressions = esquery(
      this.ast,
      `Program > VariableDeclaration[declarations.length=1] > ` +
    ...
    for (let i = 0; i < memberExpressions.length; i++) {
      ...
      const functionExpression = astOperations.ASTRelations.getParentNodeOfType(
        memberExpression,
        'FunctionExpression',
      );
...
class StringArrayRotateTemplateNodeFinder extends NodeFinder {
  ...
  findNode() {
    ...
    const possibleArrayRotateNodes =
      this._findPossibleStringArrayRotateTemplateNodes();
    let nrSuitableNodes = 0;
    possibleArrayRotateNodes.forEach((arrayRotateFunctionTemplateNode) => {
      if (
        this._isStringArrayRotateFunctionTemplate(
          arrayRotateFunctionTemplateNode,
        )
      ) {
  ...
  _findPossibleStringArrayRotateTemplateNodes() {
    const suitableRotateNodes = [];

    const identifierNodes = esquery(
      this.ast,
      `ExpressionStatement CallExpression[arguments.length=2] >` +
        `Identifier[name=${this.stringArrayTemplateIdentifierName}]`,
    );

    identifierNodes.forEach((node) => {
      const callExpressionNode = astOperations.ASTRelations.getParentNodeOfType(
        node,
        'CallExpression',
      );
  ...
  _isStringArrayRotateFunctionTemplate(node) {
    ...
    for (let i = 0; i < this.wrapperNames.length; i++) {
      ...
      const identifierStringArrayCallsWrapperTemplate = esquery(
        functionExpressionNode,
        `Identifier[name=${wrapperName}]`,
      );
    ...
    if (
      !functionExpressionNode.params ||
      functionExpressionNode.params.length != 2 ||
    ...
    const identifierInsideWhileNodes = esquery(
      functionExpressionNode,
      `WhileStatement > BlockStatement > TryStatement ` +
    ...
    const whileNode = astOperations.ASTRelations.getParentNodeOfType(
      identifierInsideWhileNodes[0],
      'WhileStatement',
    );

    const parseIntNodes = esquery(
      whileNode,
      `CallExpression > Identifier[name='parseInt']`,
    );
    if (parseIntNodes.length < 4) {
      return false;
    }

    const breakStatementNode = esquery(
      whileNode,
      `TryStatement > BlockStatement > IfStatement BreakStatement`,
    );
    if (breakStatementNode.length != 1) {
      return false;
    }

    const catchClauseExpressionStatements = esquery(
      whileNode,
      `TryStatement > CatchClause > BlockStatement > ExpressionStatement`,
    );
    const pushIdentifiers = esquery(
      catchClauseExpressionStatements[0],
      `MemberExpression > Literal[value='push']`,
    );
    const pushLiterals = esquery(
      catchClauseExpressionStatements[0],
      `MemberExpression > Identifier[name='push']`,
    );
    if (pushIdentifiers.length != 1 && pushLiterals.length != 1) {
      return false;
    }
    const shiftIdentifiers = esquery(
      catchClauseExpressionStatements[0],
      `MemberExpression > Literal[value='shift']`,
    );
    const shiftLiterals = esquery(
      catchClauseExpressionStatements[0],
      `MemberExpression > Identifier[name='shift']`,
    );
    if (shiftIdentifiers.length != 1 && shiftLiterals.length != 1) {
      return false;
    }
  ...
  _findArrayRotateTemplateForOlderObfuscatorVersions() {
    const functionExpressions = esquery(
      this.ast,
      `Program > ExpressionStatement > CallExpression[arguments.length=2] ` +
    ...
    for (let i = 0; i < functionExpressions.length; i++) {
      ...
      if (literalsShift.length != 0) {
        ...
        const identifiersShift = esquery(
          functionExpression,
          `Identifier[name='shift']`,
        );
    ...
    if (nrSuitableNodes > 1) {
      this.logger.warn(
        `Found more than one string array rotate nodes (old version).`,
      );
      this.stringArrayRotateNode = null;
    }
    ...
    if (nrSuitableNodes == 0) {
      this.logger.warn(
        `Could not find string array rotate node (old version).`,
      );
    }
  }
...
class StringArrayNodesFinder {
  ...
  _findNodeWithStringArrayTemplate() {
    const finder = new StringArrayTemplateNodeFinder(
      this.logger,
    ...
    this.logger.info(
      `[stage_03_stringarray.js] stringArrayTemplateFunctionName = ` +
  ...
  _findNodesWithStringArrayCallWrapperTemplate() {
    ...
    const finder = new StringArrayWrapperTemplateNodesFinder(
      this.logger,
      this.obfuscatedSourceCode,
      this.ast,
      this.stringArrayTemplateFunctionName,
    ...
    this.stringArrayCallWrapperTemplateFunctionNames =
      finder.getIdentifierName();
    this.logger.info(
      `[stage_03_stringarray.js] stringArrayCallsWrapperTemplateFunctionName = ` +
  ...
  foundStringArraySpecificNodes() {
    return (
      this.argv.stringArrayCmdLineSpecificNodesInfo ||
      (this.nodeWithStringArrayTemplate &&
        this.stringArrayTemplateFunctionName &&
  ...
  _getNodeWithStringArrayRotateFunctionTemplate() {
    const finder = new StringArrayRotateTemplateNodeFinder(
      this.logger,
      this.obfuscatedSourceCode,
      this.ast,
      this.stringArrayTemplateFunctionName,
    ...
    if (this.nodeWithStringArrayRotateFunction) {
      this.logger.info(
        `[stage_03_stringarray.js] Found nodeWithStringArrayRotateFunction.`,
      );
    }
  }
  ...
  _addCommandLineStringArrayCodeToExecutionContext() {
    this.logger.info(
      `[CommandLineInput] Addding found stringarray template to execution context.`,
    );
    this.logger.debug(
      `[EVAL][CommandLineInput][StringArray] ${this.argv.stringArrayCmdLineSpecificNodesInfo.stringArrayTemplateCode}`,
    );
    eval.call(
      module,
    ...
    this.logger.info(
      `[CommandLineInput] Addding found stringarray calls wrappers templates to execution context.`,
    );
    this.logger.debug(
      `[EVAL][CommandLineInput][StringArrayWrappers] ${this.argv.stringArrayCmdLineSpecificNodesInfo.stringArrayWrapperTemplates}`,
    );
    eval.call(
      module,
    ...
    if (this.argv.stringArrayCmdLineSpecificNodesInfo.stringArrayRotate) {
      this.logger.info(
        `[CommandLineInput] Addding found stringarray rotate template to execution context.`,
      );
      this.logger.debug(
        `[EVAL][CommandLineInput][StringArrayRotate] ${this.argv.stringArrayCmdLineSpecificNodesInfo.stringArrayRotate}`,
      );
      eval.call(
        module,
  ...
  _addStringArrayTemplateToExecutionContext() {
    this.logger.info(
      `Addding found stringarray template to execution context.`,
    );
    const correspondingCode = this.obfuscatedSourceCode.substring(
      this.nodeWithStringArrayTemplate.range[0],
      this.nodeWithStringArrayTemplate.range[1],
    );
    let codeToAdd = correspondingCode.replace(
      /^\s*function\s+[^(]+\(/,
      'function (',
    );
  ...
  _addStringArrayCallWrappersToExecutionContext() {
    this.logger.info(
      `Addding found stringarray calls wrappers templates to execution context.`,
    );
    for (
      let i = 0;
      i < this.nodesWithStringArrayCallWrapperTemplate.length;
      i++
    ) {
      ...
      if (correspondingCode.startsWith('var ')) {
        ...
        codeToAdd = correspondingCode.replace(
          /^\s*function\s+[^(]+\(/,
          'function (',
        );
      ...
      this.logger.debug(
        `[EVAL][NodesFinder][StringArrayWrappers] ${codeToAdd}`,
      );
      eval.call(module, codeToAdd);
    }
  }
  ...
  _addStringArrayRotateFunctionToExecutionContext() {
    ...
    this.logger.info(
      `Addding found stringarray rotate template to execution context.`,
    );
    let codeStringArrayRotateFunction = this.obfuscatedSourceCode.substring(
      this.nodeWithStringArrayRotateFunction.range[0],
      this.nodeWithStringArrayRotateFunction.range[1],
    );
    if (codeStringArrayRotateFunction[0] != '(') {
      codeStringArrayRotateFunction = `(${codeStringArrayRotateFunction});`;
    }
    this.logger.debug(
      `[EVAL][NodesFinder][StringArrayRotate] ${codeStringArrayRotateFunction}`,
    );
  ...
  _removeStringArrayNodesFoundInAST() {
    astOperations.ASTModifier.removeSingleNode(
      this.nodeWithStringArrayTemplate,
      this.logger,
      this.obfuscatedSourceCode,
    );
    for (
      ...
      astOperations.ASTModifier.removeSingleNode(
        this.nodesWithStringArrayCallWrapperTemplate[i],
        this.logger,
        this.obfuscatedSourceCode,
      );
    }
    if (this.nodeWithStringArrayRotateFunction) {
      astOperations.ASTModifier.removeSingleNode(
        this.nodeWithStringArrayRotateFunction,
        this.logger,
        this.obfuscatedSourceCode,
      );
    }
...
class StringArrayCallsReplacer {
  constructor(
    ...
    obfuscatedSourceCode,
    ast,
    stringArrayCallWrapperTemplateFunctionNames,
  ) {
    this.logger = logger;
    this.obfuscatedSourceCode = obfuscatedSourceCode;
    this.ast = ast;
  ...
  replace() {
    ...
    estraverse.traverse(this.ast, {
      enter: function (node) {
        if (
          astOperations.NodeChecker.nodeHasBodyWithLexicalScopeStringArrayCallsReplace(
            node,
          )
        ) {
          i++;
          ...
          try {
            ...
            thisObj.logger.error(
              `[stage_03_stringarray.js] Deobfuscation when fixing` +
  ...
  _initializeVariableAndFunctionWrappers() {
    for (
      let i = 0;
      i < this.stringArrayCallWrapperTemplateFunctionNames.length;
      i++
    ) {
      const wrapperName = this.stringArrayCallWrapperTemplateFunctionNames[i];
  ...
  _fixHostNodes(node) {
    ...
    for (let i = 0; i < body.length; i++) {
      const variableDeclaration = body[i];
      if (
        !variableDeclaration ||
      ...
      const variableDeclaratorsWithHostNodes =
        this._getVariableDeclaratorsWithHostNodes(
          variableDeclaration.declarations,
        );
      const nodesToRemoveForCurrentVariableDeclarationNode =
        this._getNodesRelatedToHostNodesThatNeedToBeRemoved(
          variableDeclaration,
          variableDeclaratorsWithHostNodes,
        );
    ...
    astOperations.ASTModifier.removeMultipleNodesWithSameParent(
      nodesToRemove,
      this.logger,
      this.obfuscatedSourceCode,
    );
  ...
  _getVariableDeclaratorsWithHostNodes(declarations) {
    const variableDeclaratorHostNodes = [];
    for (let i = 0; i < declarations.length; i++) {
      ...
      if (
        !this._isVariableDeclaratorAssociatedWithHostNode(variableDeclarator)
      ) {
        break;
      }
  ...
  _getNodesRelatedToHostNodesThatNeedToBeRemoved(
    variableDeclaration,
    variableDeclaratorsWithHostNodes,
  ) {
    const nodesToRemove = [];
    if (
      variableDeclaration.declarations.length ==
      variableDeclaratorsWithHostNodes.length
    ) {
  ...
  _addHostNodesToExecutionContext(variableDeclaratorsWithHostNodes) {
    for (let i = 0; i < variableDeclaratorsWithHostNodes.length; i++) {
      ...
      const codeToEval =
        `var ` +
        `${this.obfuscatedSourceCode.substring(
  ...
  _replaceStringArrayCallsUsingVariableWrappers(node) {
    const variableDeclaratorWrappers =
      this._getVariableDeclaratorWrappersInScope(node);
    this._addVariableDeclaratorWrappersToExecutionContext(
      variableDeclaratorWrappers,
    );
    this._replaceVariableDeclaratorWrappersCalls(
      node,
      variableDeclaratorWrappers,
    );
  ...
  _getVariableDeclaratorWrappersInScope(node) {
    ...
    const body = this._getNodeBody(node);
    if (!body) {
      return wrappers;
    }
    ...
    for (let i = 0; i < body.length; i++) {
      ...
      const variableDeclaratorWrappers =
        this._getVariableDeclaratorWrappersFromStatement(statement);
  ...
  _getVariableDeclaratorWrappersFromStatement(node) {
    ...
    for (let i = 0; i < node.declarations.length; i++) {
      ...
      const allVariableWrapperNames =
        this._getIdentifierNamesForVariableDeclaratorNodes(
          this.allVariableWrappers,
        );
      ...
      if (initNode.type == 'Identifier') {
        ...
        const callExpressionNodes = esquery(
          initNode,
          `FunctionExpression[params.length>0] > BlockStatement > ` +
            `ReturnStatement > CallExpression[callee.type='Identifier']`,
        );
  ...
  _getIdentifierNamesForVariableDeclaratorNodes(variableDeclaratorNodes) {
    ...
    for (let i = 0; i < variableDeclaratorNodes.length; i++) {
      ...
      if (typeof variableDeclarator == 'string') {
        ...
        const name =
          this._getIdentifierNameForVariableWrapper(variableDeclarator);
        if (name) {
          names.push(name);
        }
      }
  ...
  _addVariableDeclaratorWrappersToExecutionContext(wrappers) {
    for (let i = 0; i < wrappers.length; i++) {
      ...
      const wrapperCode = `var ${this.obfuscatedSourceCode.substring(
        wrapper.range[0],
        wrapper.range[1],
      )}`;
  ...
  _replaceVariableDeclaratorWrappersCalls(node, currentNodeVariableWrappers) {
    const nodesWithCallsToReplace = this._getVariableWrapperCallsInScope(
      node,
      currentNodeVariableWrappers,
    );
    for (let i = 0; i < nodesWithCallsToReplace.length; i++) {
      const callExpressionNode = nodesWithCallsToReplace[i];
      this.logger.debug(
        `[EVAL][VariableWrapperReplace] ${this.obfuscatedSourceCode.substring(
          callExpressionNode.range[0],
          callExpressionNode.range[1],
        )}`,
      );
      const stringToReplaceWith =
        astOperations.NodeEvaller.evalCallExpressionNodeBasedOnSourceCode(
          callExpressionNode,
          this.obfuscatedSourceCode,
        );
      astOperations.ASTModifier.replaceNode(
        callExpressionNode,
        {
          type: 'Literal',
  ...
  _getVariableWrapperCallsInScope(nodeWithBody, currentNodeVariableWrappers) {
    ...
    const curentNodeWrapperNames =
      this._getIdentifierNamesForVariableDeclaratorNodes(
        currentNodeVariableWrappers,
      );
    const allWrapperNames = this._getIdentifierNamesForVariableDeclaratorNodes(
      this.allVariableWrappers,
    );
    const wrapperNamesSearchedInCurrentScope =
      this._getVariableWrapperNamesToSearchInCurrentScope(
        nodeWithBody,
        curentNodeWrapperNames,
        allWrapperNames,
      );
    ...
    estraverse.traverse(nodeWithBody, {
      enter: function (node, parent) {
        if (
          !firstNode &&
          astOperations.NodeChecker.nodeHasBodyWithLexicalScope(node)
        ) {
          this.skip();
        }
        firstNode = false;
        ...
        if (node.type == 'Identifier' && parent.type == 'CallExpression') {
          ...
          if (
            wrapperNamesSearchedInCurrentScope.includes(identifierName) &&
  ...
  _getVariableWrapperNamesToSearchInCurrentScope(
    nodeWithBody,
    curentNodeWrapperNames,
    allWrapperNames,
  ) {
    ...
    for (let i = 0; i < allWrapperNames.length; i++) {
      ...
      if (
        curentNodeWrapperNames.includes(wrapperName) ||
        nodeWithBody.type == 'Program'
      ) {
      ...
      if (
        !this._wrapperIsRedefinedInCurrentScope(wrapperName, nodeWithBody) &&
        (typeof variableDeclaratorWrapper == 'string' ||
          this._wrapperWasDefinedInAScopeThatIsParentOfCurrentScope(
  ...
  _wrapperIsRedefinedInCurrentScope(wrapperName, nodeWithBody) {
    return (
      this._wrapperIsRedefinedInCurrentScopeAsNewVariable(
        wrapperName,
        nodeWithBody,
      ) ||
      this._wrapperIsRedefinedInCurrentScopeAsNewFunction(
        wrapperName,
        nodeWithBody,
      ) ||
      this._wrapperIsRedefinedInCurrentScopeAsAssignmentExpression(
        wrapperName,
        nodeWithBody,
      )
    );
  ...
  _wrapperIsRedefinedInCurrentScopeAsAssignmentExpression(
    wrapperName,
    nodeWithBody,
  ) {
    let firstNode = true;
  ...
  _wrapperWasDefinedInAScopeThatIsParentOfCurrentScope(wrapper, nodeWithBody) {
    const wrapperLexicalScope =
      astOperations.ASTRelations.getParentNodeWithLexicalScope(wrapper);

    let node = nodeWithBody;
    let parent = null;
    do {
  ...
  _removeVariableDeclaratorWrappers(wrappers) {
    for (let i = 0; i < wrappers.length; i++) {
      ...
      if (
        wrapper.parent.type == 'VariableDeclaration' &&
        wrapper.parent.declarations.length == 1
      ) {
  ...
  _replaceStringArrayCallsUsingFunctionWrappers(nodeWithBody) {
    const currentNodeFunctionWrappers =
      this._getFunctionDeclarationWrappersInScope(nodeWithBody);
    this._addFunctionDeclarationWrappersToExecutionContext(
      currentNodeFunctionWrappers,
    );
    this._replaceFunctionDeclarationWrappersCalls(
      nodeWithBody,
      currentNodeFunctionWrappers,
    );
  ...
  _getFunctionDeclarationWrappersInScope(nodeWithBody) {
    ...
    const body = this._getNodeBody(nodeWithBody);
    if (!body) {
      return wrappers;
    }
    ...
    for (let i = 0; i < body.length; i++) {
      const statement = body[i];
      if (
        this._nodeIsFunctionDeclarationWrapper(statement) ||
  ...
  _getNodeBody(nodeWithBody) {
    ...
    if (
      nodeWithBody.type == 'Program' ||
      nodeWithBody.type == 'WhileStatement'
    ) {
  ...
  _isFunctionWrapperNode(node) {
    ...
    const allFunctionWrapperNames =
      this._getIdentifierNamesForFunctionWrapperNodes(this.allFunctionWrappers);
  ...
  _addFunctionDeclarationWrappersToExecutionContext(wrappers) {
    for (let i = 0; i < wrappers.length; i++) {
      const wrapper = wrappers[i];
      const wrapperCode = this.obfuscatedSourceCode.substring(
        wrapper.range[0],
        wrapper.range[1],
      );
  ...
  _replaceFunctionDeclarationWrappersCalls(
    nodeWithBody,
    currentNodeFunctionWrappers,
  ) {
    const nodesWithCallsToReplace = this._getFunctionWrapperCallsInScope(
      nodeWithBody,
      currentNodeFunctionWrappers,
    );
    for (let i = 0; i < nodesWithCallsToReplace.length; i++) {
      const callExpressionNode = nodesWithCallsToReplace[i];
      this.logger.debug(
        `[EVAL][FunctionWrapperReplace] ${this.obfuscatedSourceCode.substring(
          callExpressionNode.range[0],
          callExpressionNode.range[1],
        )}`,
      );
  ...
  _getFunctionWrapperCallsInScope(nodeWithBody, currentNodeFunctionWrappers) {
    ...
    const curentNodeWrapperNames =
      this._getIdentifierNamesForFunctionWrapperNodes(
        currentNodeFunctionWrappers,
      );
    const allWrapperNames = this._getIdentifierNamesForFunctionWrapperNodes(
      this.allFunctionWrappers,
    );
    const wrapperNamesForSearching =
      this._getFunctionWrapperNamesToSearchInCurrentScope(
        nodeWithBody,
        curentNodeWrapperNames,
        allWrapperNames,
      );
    ...
    estraverse.traverse(nodeWithBody, {
      enter: function (node, parent) {
        ...
        if (node.type == 'Identifier' && parent.type == 'CallExpression') {
          const identifierName = node.name;
          if (
            wrapperNamesForSearching.includes(identifierName) &&
            parent.callee.name == identifierName
          ) {
            wrapperCalls.push(parent);
          }
  ...
  _getFunctionWrapperNamesToSearchInCurrentScope(
    nodeWithBody,
    curentNodeWrapperNames,
    allWrapperNames,
  ) {
...
class StringArraySyntheticTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  _findStringArraySpecificNodes() {
    this.stringArrayNodesFinder = new StringArrayNodesFinder(
      this.logger,
      this.obfuscatedSourceCode,
      this.ast,
      this.argv,
    ...
    this.stringArrayCallWrapperTemplateFunctionNames =
      this.stringArrayNodesFinder.getStringArrayCallWrapperFunctionNames();
  ...
  _replaceStringArrayCallsWithCorrespondingString() {
    this.logger.info(
      'Replacing stringarray calls with actual strings - START.',
    );
    this.stringArrayCallsReplacer = new StringArrayCallsReplacer(
      this.logger,
      this.obfuscatedSourceCode,
      this.ast,
      this.stringArrayCallWrapperTemplateFunctionNames,
    ...
    this.logger.info(
      'Replacing stringarray calls with actual strings - FINISH.',
    );
  }
}
