...
class StageControlFlowFlattening extends stageDeobfuscator.StageDeobfuscator {
  ...
  _initiateTransformers() {
    this.transformers = {
      ControlFlowFlatteningFunctionControlFlowTransformer:
        ControlFlowFlatteningFunctionControlFlowTransformer,
      ControlFlowFlatteningCustomCodeHelpersTransformer:
        ControlFlowFlatteningCustomCodeHelpersTransformer,
      ControlFlowFlatteningBlockStatementControlFlowTransformer:
        ControlFlowFlatteningBlockStatementControlFlowTransformer,
    };
  }
}
...
class ControlFlowFlatteningFunctionControlFlowTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  deobfuscate() {
    const controlFlowReplacerClasses = {
      ControlFlowFlatteningBinaryExpressionControlFlowReplacer:
        ControlFlowFlatteningBinaryExpressionControlFlowReplacer,
      ControlFlowFlatteningCallExpressionControlFlowReplacer:
        ControlFlowFlatteningCallExpressionControlFlowReplacer,
      ControlFlowFlatteningLogicalExpressionControlFlowReplacer:
        ControlFlowFlatteningLogicalExpressionControlFlowReplacer,
      ControlFlowFlatteningStringLiteralControlFlowReplacer:
        ControlFlowFlatteningStringLiteralControlFlowReplacer,
    };
    let i = 0;
    for (const className in controlFlowReplacerClasses) {
      const replacerClass = controlFlowReplacerClasses[className];
      const replacerObject = new replacerClass(
        this.logger,
      ...
      this.logger.info(
        `[stage_07_controflowflattening.js] ${
          i + 1
        }/4) Replacer '${className}'.`,
      );
      console.log(
        `[stage_07_controflowflattening.js] ${
          i + 1
      ...
      try {
        this.sourceCodeBeforeReplacer =
          astOperations.ASTSourceCodeOperations.generateSourceCodeFromAST(
            this.ast,
          );
        ...
        this.logger.error(
          `[stage_07_controlflowflattening.js] Deobfuscation error for ControlFlowFlattening ` +
            `
  ...
  _removeEmptyObjectExpressionDeclarations() {
    const objectExpressionNodes = esquery(
      this.ast,
      `VariableDeclaration > VariableDeclarator > ` +
        `ObjectExpression[properties.length=0]`,
    );
    objectExpressionNodes.forEach((objectExpression) => {
      const variableDeclarator = objectExpression.parent;
      const identifier = variableDeclarator.id.name;

      const objectExpressionLexicalScope =
        astOperations.ASTRelations.getParentNodeWithLexicalScope(
          objectExpression,
        );
      const identifiersWithObjExpressionName = esquery(
        objectExpressionLexicalScope,
  ...
  _displaySourceCodeBeforeAndAfterReplacer(className) {
    const sourceCodeAfterReplacer =
      astOperations.ASTSourceCodeOperations.generateSourceCodeFromAST(this.ast);
    ...
    this._writeTransformationReplcerDiffFiles(
      className,
  ...
  _writeTransformationReplcerDiffFiles(className, sourceCodeAfterReplacer) {
    ...
    if (this.argv.writedifffiles) {
      const fileName = `transform_replacer_${(
        '0' + this.argv.replacerNumber
      ).slice(-2)}_${className}`;
      usefulModule.writeTextFile(
        `${fileName}.in`,
        this.sourceCodeBeforeReplacer,
      );
...
class FunctionControlFlowFlatteningReplacer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  _restore(node) {
    const found = this._replaceMemberExpressionCalls(node);
    if (found) {
  ...
  _getLexicalScopeOfObjectExpression(node) {
    const objectExpressionNode = astOperations.ASTRelations.getParentNodeOfType(
      node,
      'ObjectExpression',
    );
    return astOperations.ASTRelations.getParentNodeWithLexicalScope(
      objectExpressionNode,
    );
  }

  _getMemberExpressionNames(node) {
    const propertyNode = astOperations.ASTRelations.getParentNodeOfType(
      node,
      'Property',
    );
...
class ControlFlowFlatteningBinaryExpressionControlFlowReplacer extends FunctionControlFlowFlatteningReplacer {
  ...
  deobfuscate() {
    ...
    do {
      nrIterations++;
      console.log(
        `[stage_07][ControlFlowFlatteningBinaryExpressionControlFlowReplacer] nrIterations = ${nrIterations}.`,
      );
      var foundControlFlowFlattenedBinaryExpresion = false;
      const binaryExpressionNodes = esquery(
        this.ast,
      ...
      binaryExpressionNodes.forEach((binaryExpressionNode) => {
        ...
        if (nrExpr == 1 || nrExpr % 100 == 0) {
          console.log(
            `[stage_07][ControlFlowFlatteningBinaryExpressionControlFlowReplacer] nrExpr = ${nrExpr}/${totalBinaryExpressions}.`,
          );
        }
  ...
  _binaryExpressionWasCFFlattened(node) {
    const blockStatementNode = astOperations.ASTRelations.getParentNodeOfType(
      node,
      'BlockStatement',
    );
    ...
    if (
      node.left.name != functionExpressionNode.params[0].name ||
  ...
  _restore(node) {
    const foundControlFlowFlattenedBinaryExpresion =
      this._replaceMemberExpressionCalls(node);
  ...
  _replaceMemberExpressionCalls(binaryExpressionNode) {
    let foundControlFlowFlattenedBinaryExpresion = false;

    const objectExpressionLexicalScope =
      this._getLexicalScopeOfObjectExpression(binaryExpressionNode);
    const memberExpressionNames =
      this._getMemberExpressionNames(binaryExpressionNode);

    const memberExpressionNodes = esquery(
      objectExpressionLexicalScope,
...
class ControlFlowFlatteningCallExpressionControlFlowReplacer extends FunctionControlFlowFlatteningReplacer {
  ...
  deobfuscate() {
    ...
    do {
      nrIterations++;
      console.log(
        `[stage_07][ControlFlowFlatteningCallExpressionControlFlowReplacer] nrIterations = ${nrIterations}.`,
      );
      ...
      callExpressionNodes.forEach((callExpression) => {
        ...
        if (currentNrCalls == 1 || currentNrCalls % 100 == 0) {
          console.log(
            `[stage_07][ControlFlowFlatteningCallExpressionControlFlowReplacer] currentNrCalls = ${currentNrCalls}/${totalNrCalls}.`,
          );
        }
  ...
  _callExpressionWasCFFlattened(callExpression) {
    ...
    if (
      callExpression.arguments.length &&
      !astOperations.NodeComparer.arraysWithIdentifierNodesAreEqual(
        functionExpression.params.slice(1),
  ...
  _restore(callExpression) {
    const foundControlFlowFlattenedCallExpression =
      this._replaceMemberExpressionCalls(callExpression);
  ...
  _replaceMemberExpressionCalls(callExpression) {
    let foundControlFlowFlattenedCallExpression = false;

    const objectExpressionLexicalScope =
      this._getLexicalScopeOfObjectExpression(callExpression);
    const memberExpressionNames =
      this._getMemberExpressionNames(callExpression);
...
class ControlFlowFlatteningLogicalExpressionControlFlowReplacer extends FunctionControlFlowFlatteningReplacer {
  ...
  deobfuscate() {
    ...
    do {
      nrIterations++;
      console.log(
        `[stage_07][ControlFlowFlatteningLogicalExpressionControlFlowReplacer] nrIterations = ${nrIterations}.`,
      );
      var foundControlFlowFlattenedLogicalExpression = false;

      const logicalExpressionNodes = esquery(
        this.ast,
        `VariableDeclaration > VariableDeclarator > ObjectExpression > ` +
      ...
      logicalExpressionNodes.forEach((logicalExpressiion) => {
        ...
        if (
          currentNrLogicalExpressions == 1 ||
          currentNrLogicalExpressions % 100 == 0
        ) {
          console.log(
            `[stage_07][ControlFlowFlatteningLogicalExpressionControlFlowReplacer] currentNrLogicalExpressions = ` +
              `${currentNrLogicalExpressions}/${totalNrLogicalExpressions}.`,
          );
        }
        if (
          !this._logicalExpressionWasControlFlowFlattened(logicalExpressiion)
        ) {
          return;
        }
  ...
  _logicalExpressionWasControlFlowFlattened(logicalExpression) {
    ...
    if (
      logicalExpression.left.type != 'Identifier' ||
      logicalExpression.right.type != 'Identifier'
    ) {
      return false;
    }

    if (
      functionExpression.params[0].type != 'Identifier' ||
      functionExpression.params[1].type != 'Identifier'
    ) {
      return false;
    }

    if (
      logicalExpression.left.name != functionExpression.params[0].name ||
  ...
  _restore(logicalExpression) {
    const foundControlFlowFlattenedLogicalExpression =
      this._replaceMemberExpressionCalls(logicalExpression);
  ...
  _replaceMemberExpressionCalls(logicalExpression) {
    let foundControlFlowFlattenedLogicalExpression = false;

    const objectExpressionLexicalScope =
      this._getLexicalScopeOfObjectExpression(logicalExpression);
    const memberExpressionNames =
      this._getMemberExpressionNames(logicalExpression);
    const memberExpressionNodes = esquery(
      objectExpressionLexicalScope,
      `CallExpression > MemberExpression` +
        `[object.type='Identifier'][object.name=${memberExpressionNames.objectName}][property.type='Identifier']` +
...
class ControlFlowFlatteningStringLiteralControlFlowReplacer extends FunctionControlFlowFlatteningReplacer {
  ...
  deobfuscate() {
    ...
    do {
      nrIterations++;
      console.log(
        `[stage_07][ControlFlowFlatteningStringLiteralControlFlowReplacer] nrIterations = ${nrIterations}.`,
      );
      var foundControlFlowFlattenedCallExpression = false;
      ...
      literalNodes.forEach((stringLiteralNode) => {
        ...
        if (currentNrLiteralNodes == 1 || currentNrLiteralNodes % 100 == 0) {
          console.log(
            `[stage_07][ControlFlowFlatteningCallExpressionControlFlowReplacer] currentNrCalls = ` +
  ...
  _restore(stringLiteralNode) {
    const foundControlFlowFlattenedStringLiteral =
      this._replaceMemberExpressionCalls(stringLiteralNode);
  ...
  _replaceMemberExpressionCalls(stringLiteralNode) {
    let foundControlFlowFlattenedStringLiteral = false;

    const objectExpressionLexicalScope =
      this._getLexicalScopeOfObjectExpression(stringLiteralNode);
    const memberExpressionNames =
      this._getMemberExpressionNames(stringLiteralNode);
    const memberExpressionNodes = esquery(
      objectExpressionLexicalScope,
...
class ControlFlowFlatteningBlockStatementControlFlowTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  _deobfuscateNodes(astNodes) {
    ...
    for (let i = 0; i < astNodes.length; i++) {
      const controllerNodes = this._getControllerNodes(astNodes[i]);
  ...
  _getControllerNodes(ast) {
    ...
    const possibleControllerNodes = esquery(
      ast,
      `VariableDeclaration > VariableDeclarator > CallExpression[arguments.length=1] > ` +
  ...
  _isControllerNodeForBlockStatement(memberExpressionNode) {
    const callExpression = memberExpressionNode.parent;
    if (
    ...
    if (
      typeof controlFlowOrderStr != 'string' ||
      !controlFlowOrderStr.match(/^(?:\d+\|)+\d+/)
    ) {
      return false;
    }
    ...
    const whileTrueNode = this._getWhileTrueNode(memberExpressionNode);
    if (!whileTrueNode) {
      return false;
    }

    const indexerNode = this._getIndexerNode(whileTrueNode);
    if (!indexerNode) {
      return false;
    }
  ...
  _getWhileTrueNode(controllerNodeMemberExpression) {
    const variableDeclarator = astOperations.ASTRelations.getParentNodeOfType(
      controllerNodeMemberExpression,
      'VariableDeclarator',
    );
    const controllerNodeName = variableDeclarator.id.name;

    const controllerNodeLexicalScope =
      astOperations.ASTRelations.getParentNodeWithLexicalScope(
        controllerNodeMemberExpression,
      );
    const whileTrueNodes = esquery(
      controllerNodeLexicalScope,
      `WhileStatement[test.type='Literal']` +
    ...
    if (
      this._isGetWhileTrueNodeBlockValid(
        whileTrueNodes[0],
        controllerNodeMemberExpression,
      )
    ) {
  ...
  _isGetWhileTrueNodeBlockValid(whileTrueNode, controllerNodeMemberExpression) {
    const controlFlowOrder = this._getControlFlowOrder(
      controllerNodeMemberExpression,
    );
  ...
  _getIndexerNode(whileTrueNode) {
    const switchStatementNode = whileTrueNode.parent.body[0];
    const indexerNodeName =
      switchStatementNode.discriminant.property.argument.name;

    const whileTrueNodeLexicalScope =
      astOperations.ASTRelations.getParentNodeWithLexicalScope(whileTrueNode);
    const indexerNodes = esquery(
      whileTrueNodeLexicalScope,
      `VariableDeclaration > VariableDeclarator[id.type='Identifier']` +
  ...
  _deobfuscateControllerNode(controllerNodeMemberExpression) {
    const controllerVariableDeclarationNode =
      astOperations.ASTRelations.getParentNodeOfType(
        controllerNodeMemberExpression,
        'VariableDeclaration',
      );

    const whileTrueBreakStatementNode = this._getWhileTrueNode(
      controllerNodeMemberExpression,
    );
    if (whileTrueBreakStatementNode == null) {
      return;
    }
    const indexerNode = this._getIndexerNode(whileTrueBreakStatementNode);

    const whileStatementNode = astOperations.ASTRelations.getParentNodeOfType(
      whileTrueBreakStatementNode,
      'WhileStatement',
    );
    ...
    const deobfuscatedStatements = [];
    let foundError = false;
    controlFlowOrder.forEach((nextCase) => {
      const switchCaseNodes = esquery(
        whileStatementNode,
        `WhileStatement > BlockStatement > SwitchStatement > ` +
      ...
      if (switchCaseNodes.length != 1) {
        for (let i = 0; i < switchCaseNodes.length; i++) {
          const tempSwitchCase = switchCaseNodes[i];
          const tempWhileStatement =
            astOperations.ASTRelations.getParentNodeOfType(
              tempSwitchCase,
              'WhileStatement',
            );
        ...
        if (!switchCase) {
          ...
          this.logger.warn(
            `[stage_07_controlflowflattening.js] Something is wrong while deobfuscating switch cases ` +
      ...
      if (
        switchCase.consequent[switchCase.consequent.length - 1].type !=
        'ContinueStatement'
      ) {
        deobfuscatedStatements.push(
    ...
    let blockStatementNode = astOperations.ASTRelations.getParentNodeOfType(
      whileStatementNode,
      'BlockStatement',
    );
    if (!blockStatementNode) {
      blockStatementNode = astOperations.ASTRelations.getParentNodeOfType(
        whileStatementNode,
        'Program',
      );
    }
    astOperations.ASTModifier.insertNodesAfter(
      whileStatementNode,
      deobfuscatedStatements,
      this.logger,
      this.obfuscatedSourceCode,
    );
    ...
    astOperations.ASTModifier.removeSingleNode(
      controllerVariableDeclarationNode,
      this.logger,
      this.obfuscatedSourceCode,
    );
    astOperations.ASTModifier.removeSingleNode(
