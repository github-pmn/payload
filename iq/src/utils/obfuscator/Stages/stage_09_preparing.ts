...
class PreparingCustomCodeHelpersTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  deobfuscate() {
    ...
    for (const className in removerClasses) {
      ...
      const removerObject = new removerClass(
        this.logger,
      ...
      try {
        ...
        this.logger.error(
          `[stage_09_preparing.js] Deobfuscation error for Preparing remover ` +
...
class ConsoleOutputDisableRemover {
  ...
  deobfuscate() {
    ...
    for (let i = 0; i < literalNodes.length; i++) {
      ...
      const functionExpressionParent =
      ...
      this.logger.info(
        `Found 'disableConsoleOutput' related nodes and removing them.`,
      );
  ...
  _removeCalls(variableDeclaration) {
    ...
    const identifier =
      astOperations.ASTUtility.getVariableNameFromVariableDeclaration(
        variableDeclaration,
      );
    if (!identifier) {
      return false;
    ...
    const callExpressionNodes = esquery(
      this.ast,
      `CallExpression[callee.type='Identifier'][callee.name=${identifier}]`,
    );
...
class SelfDefendingRemover {
  ...
  deobfuscate() {
    ...
    let method2SelfDefending = false;
    if (literalNodes.length == 0) {
      literalNodes = esquery(
        this.ast,
        `VariableDeclaration[declarations.length=1] > VariableDeclarator > ` +
    ...
    for (let i = 0; i < literalNodes.length; i++) {
      ...
      if (method2SelfDefending) {
        functionExpressionParent =
          astOperations.ASTRelations.getParentNodeOfType(
            functionExpressionParent.parent,
            'FunctionExpression',
          );
      }
      const variableDeclaration =
      ...
      this.logger.info(
        `Found 'selfDefending' related nodes and removing them.`,
      );
      if (this._removeCalls(variableDeclaration)) {
        astOperations.ASTModifier.removeSingleNode(
          variableDeclaration,
...
class DebugProtectionRemover extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  deobfuscate() {
    const debugProtectionClasses = {
      ...
      DebugProtectionFunctionIntervalTemplate:
        DebugProtectionFunctionIntervalTemplate,
    };
    ...
    for (const className in debugProtectionClasses) {
      ...
      this.logger.info(
        `[stage_09_preparing.js] Debug protection remover '${className}'.`,
      );
      try {
        removerObject.deobfuscate();
      } catch (e) {
        this.logger.error(
          `[stage_09_preparing.js] Deobfuscation error for Debug protection remover ` +
            `class '${className}'. error = ${e}. Stack = ${e.stack}`,
        );
      }
    }
...
class DebugProtectionFunctionTemplate {
  ...
  deobfuscate() {
    const updateExpressionNodes = esquery(
      this.ast,
      `FunctionDeclaration[params.length=1] > BlockStatement > ` +
    ...
    if (updateExpressionNodes.length != 1) {
      this.logger.warn(
        `Found ${updateExpressionNodes.length} 'DebugProtectionFunctionTemplate' nodes. ` +
          `Please check.`,
      );
      return;
    }
    const updateExpression = updateExpressionNodes[0];

    let functionDeclaration = astOperations.ASTRelations.getParentNodeOfType(
      updateExpression,
      'FunctionDeclaration',
    );
    ...
    if (functionDeclaration.type != 'FunctionDeclaration') {
      this.logger.warn(
        `Something went wrong when removing DebugProtectionFunctionTemplate related nodes.`,
      );
    }

    this.logger.info(
      `Found 'DebugProtectionFunctionTemplate' related nodes and removing them.`,
    );
    astOperations.ASTModifier.removeSingleNode(
      functionDeclaration,
      this.logger,
...
class DebugProtectionFunctionCallTemplate {
  ...
  deobfuscate() {
    const literalNodes = esquery(
      this.ast,
      `ExpressionStatement > CallExpression[arguments.length=0] > ` +
    ...
    if (literalNodes.length != 1) {
      this.logger.warn(
        `Found ${literalNodes.length} 'DebugProtectionFunctionCallTemplate' nodes. ` +
          `Please check.`,
      );
      return;
    }
    const literal = literalNodes[0];

    const functionExpression = astOperations.ASTRelations.getParentNodeOfType(
      literal,
      'FunctionExpression',
    );
    const expressionStatement = astOperations.ASTRelations.getParentNodeOfType(
      functionExpression,
      'ExpressionStatement',
    );

    this.logger.info(
      `Found 'DebugProtectionFunctionCallTemplate' related nodes and removing them.`,
    );
    astOperations.ASTModifier.removeSingleNode(
      expressionStatement,
      this.logger,
...
class DebugProtectionFunctionIntervalTemplate {
  ...
  deobfuscate() {
    ...
    if (memberExpressionNodes.length != 1) {
      this.logger.warn(
        `Found ${memberExpressionNodes.length} 'DebugProtectionFunctionIntervalTemplate' nodes. ` +
          `Please check.`,
      );
      return;
    }
    const memberExpression = memberExpressionNodes[0];

    let expressionStatement = astOperations.ASTRelations.getParentNodeOfType(
      memberExpression,
      'ExpressionStatement',
    );
    expressionStatement = astOperations.ASTRelations.getParentNodeOfType(
      expressionStatement.parent,
      'ExpressionStatement',
    );

    this.logger.info(
      `Found 'DebugProtectionFunctionIntervalTemplate' related nodes and removing them.`,
    );
    astOperations.ASTModifier.removeSingleNode(
      expressionStatement,
      this.logger,
...
class DomainLockRemover {
  ...
  deobfuscate() {
    ...
    if (binaryExpressionNodes.length != 1) {
      this.logger.warn(
        `Found ${binaryExpressionNodes.length} 'DomainLockRemover' nodes. Please check.`,
      );
      return;
    }
    const binaryExpression = binaryExpressionNodes[0];

    const variableDeclaration = astOperations.ASTRelations.getParentNodeOfType(
      binaryExpression,
      'VariableDeclaration',
    );

    this.logger.info(
      `Found 'DomainLockRemover' related nodes and removing them.`,
    );
    astOperations.ASTModifier.removeSingleNode(
      variableDeclaration,
      this.logger,
...
class CallsControllerRemover {
  ...
  deobfuscate() {
    ...
    for (let i = 0; i < identifierNodes.length; i++) {
      const identifier = identifierNodes[i];

      const returnStatement = astOperations.ASTRelations.getParentNodeOfType(
        identifier,
        'ReturnStatement',
      );
      const variableDeclaration =
      ...
      this.logger.info(
        `Found 'CallsControllerRemover' related nodes and removing them.(${
          i + 1
        }/${identifierNodes.length})`,
      );
      astOperations.ASTModifier.removeSingleNode(
        variableDeclaration,
        this.logger,
