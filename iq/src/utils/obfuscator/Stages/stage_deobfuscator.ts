    ...
    this.logger = winston.createLogger({
      ...
      transports: [
        new winston.transports.File({
          filename: `debug_${this.stageName}.log`,
          level: 'debug',
          options: { flags: 'a' },
        }),
  ...
  deobfuscate() {
    ...
    for (const transformerName in this.transformers) {
      ...
      try {
        ...
        this.logger.error(
          `[stage_deobfuscator.js] Deobfuscation error for transform '${transformerName}'. ` +
...
class TransformerDeobfuscator {
  ...
  displaySourceCodeBeforeAndAfterTransformation(transformName) {
    ...
    if (
      TransformerDeobfuscator.transformNamesToSkipFromDisplaying.includes(
        transformName,
      )
    ) {
      this.logger.debug(
        `Not displaying before&after transformation for ${transformName} because ` +
          `this transform does not modify the ast.`,
      );
      return;
    }

    const sourceCodeAfterTransformation =
      astOperations.ASTSourceCodeOperations.generateSourceCodeFromAST(this.ast);
    if (sourceCodeAfterTransformation == this.sourceCodeBeforeTransformation) {
      this.logger.debug(
        `Transformation ${transformName} does not modify contents.`,
      );
      return;
    }

    this._writeTransformationDiffFiles(
      transformName,
  ...
  _checkForMissingParentsInAST(transformName) {
    ...
    if (foundMissingParent) {
      this.logger.warn(
        `Missing parent after transformation '${transformName}.'`,
      );
    } else {
      this.logger.debug(
        `All parents are present for transformation '${transformName}.'`,
      );
    }
  }
  ...
  _writeTransformationDiffFiles(transformName, sourceCodeAfterTransformation) {
    ...
    if (this.argv.writedifffiles) {
      const fileName = `transform_${('0' + this.argv.transformNumber).slice(
        -2,
      )}_${transformName}`;
