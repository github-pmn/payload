  ...
  _initiateTransformers() {
    this.transformers = {
      ...
      ConvertingCustomCodeHelpersTransformer:
        ConvertingCustomCodeHelpersTransformer,
      ConvertingObjectExpressionKeysTransformer:
        ConvertingObjectExpressionKeysTransformer,
      ConvertingTemplateLiteralTransformer:
        ConvertingTemplateLiteralTransformer,
      ConvertingObjectPatternPropertiesTransformer:
        ConvertingObjectPatternPropertiesTransformer,
      ConvertingObjectExpressionTransformer:
        ConvertingObjectExpressionTransformer,
      ConvertingNumberToNumericalExpressionTransformer:
        ConvertingNumberToNumericalExpressionTransformer,
      ConvertingMemberExpressionTransformer:
        ConvertingMemberExpressionTransformer,
...
class ConvertingSplitStringTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  _recursiveCombineStrings(node) {
    do {
      ...
      estraverse.replace(node, {
        enter: function (node) {
          if (
            thisObj._nodeIsStringSplittedInBinaryExpressionWithLiteralsOnly(
              node,
            )
          ) {
  ...
  _concatenateStringLiteralNodes(node) {
    ...
    astOperations.ASTModifier.logDebugEstraverseReplace(
      node,
      newNode,
      this.logger,
...
class ConvertingObjectExpressionKeysTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  _deobfuscateBasePropertiesExtractor() {
    const variableDeclaratorNodes = esquery(
      this.ast,
    ...
    variableDeclaratorNodes.forEach((variableDeclarator) => {
      ...
      if (nrVarDeclarators % 100 == 0 || nrVarDeclarators == 1) {
        console.log(
          `Parsing nrVarDeclarators for base properties = ${nrVarDeclarators}/${totalLen}.`,
        );
      }
      const objectExpressionNode = variableDeclarator.init;

      const expressionStatementNodes =
        this._getExpressionStatementNodesContainingMemberExpressionWithIdentifier(
          variableDeclarator,
        );

      for (let i = 0; i < expressionStatementNodes.length; i++) {
        const expressionStatement = expressionStatementNodes[i];
        if (!expressionStatement.expression) {
          continue;
        }
        const leftPartOfAssignmentExpression =
          expressionStatement.expression.left;
        const rightPartOfAssignmentExpression =
          expressionStatement.expression.right;

        const keysPath =
          this._getPropertiesOfExpressionStatementForBasePropertiesExtractor(
            leftPartOfAssignmentExpression,
          );

        try {
          if (
            this._addToObjectExpressionNodeForBasePropertiesExtractor(
              objectExpressionNode,
              keysPath,
              rightPartOfAssignmentExpression,
            )
          ) {
          ...
          this.logger.error(
            `[stage_05_converting.js] Deobfuscation error for ` +
  ...
  _getExpressionStatementNodesContainingMemberExpressionWithIdentifier(
    variableDeclarator,
  ) {
    const nodes = [];
    ...
    for (let i = 0; i < identifierNodes.length; i++) {
      ...
      if (
        this._expressionStatementWasExtractedFromBaseProperty(
          expressionStatement,
          identifierName,
        )
      ) {
  ...
  _expressionStatementWasExtractedFromBaseProperty(
    expressionStatement,
    identifier,
  ) {
    if (
  ...
  _getPropertiesOfExpressionStatementForBasePropertiesExtractor(
  ...
  _addToObjectExpressionNodeForBasePropertiesExtractor(
    objectExpressionNode,
    keysPath,
    rightPartOfAssignmentExpression,
  ) {
    const objectExpressionNodeToAddTo =
      this._getObjectExpressionNodeToAddToForBasePropertiesExtractor(
        objectExpressionNode,
        keysPath,
      );
    ...
    astOperations.ASTModifier.logDebugNode(
      '[Converting][KeysTransformer-BaseProperties][Adding property to dictionary]' +
        '[Old]',
      objectExpressionNodeToAddTo,
      this.logger,
      this.obfuscatedSourceCode,
      true,
    );
  ...
  _getObjectExpressionNodeToAddToForBasePropertiesExtractor(
    objectExpressionToAddTo,
    keysPath,
  ) {
    let searchScope = [];
    let i = 0;
    do {
      ...
      searchScope = esquery(
        objectExpressionToAddTo,
        `ObjectExpression > Property[key.value=${JSON.stringify(key)}]`,
      );
      if (searchScope.length == 0) {
        searchScope = esquery(
          objectExpressionToAddTo,
          `ObjectExpression > Property[key.name=${JSON.stringify(key)}]`,
        );
      }
  ...
  _deobfuscateObjectExpressionToVariableDeclarationExtractor() {
    ...
    variableDeclaratorNodes.forEach((variableDeclarator) => {
      ...
      if (nrVarDeclarators % 100 == 0 || nrVarDeclarators == 1) {
        console.log(
          `Parsing nrVarDeclarators for variable declaration extraction = ${nrVarDeclarators}/${totalLen}.`,
        );
      }
      ...
      const variableDeclarationLexicalScope =
        astOperations.ASTRelations.getParentNodeWithLexicalScope(
          variableDeclaration,
        );
      const extractedVariableDeclarationNodes = esquery(
        variableDeclarationLexicalScope,
      ...
      if (extractedVariableDeclarationNodes.length == 1) {
        const newIdentifier = extractedVariableDeclarationNodes[0].id.name;
        ...
        this._renameVariableInExpressions(
          variableDeclarationLexicalScope,
          oldIdentifier,
          newIdentifier,
        );
    ...
    console.log(
      `Finished parsing nrVarDeclarators for variable declaration extraction.`,
    );
  }
  ...
  _renameVariableDeclarator(variableDeclarator, newName) {
    const variableDeclaration = variableDeclarator.parent;
    astOperations.ASTModifier.logDebugNode(
      '[Converting][KeysTransformer-VariableDeclaration][Rename dictionary]' +
        '[Old]',
      variableDeclaration,
      this.logger,
      this.obfuscatedSourceCode,
  ...
  _renameVariableInExpressions(
    variableDeclarationLexicalScope,
    oldIdentifier,
    newIdentifier,
  ) {
    const identifierExpressionStatementNodes = esquery(
      variableDeclarationLexicalScope,
      `ExpressionStatement > ` +
        `AssignmentExpression[operator='='][left.type='MemberExpression'][left.property.type='Literal'] ` +
    ...
    identifierExpressionStatementNodes.forEach((identifierNode) => {
      ...
      astOperations.ASTModifier.logDebugNode(
        '[Converting][KeysTransformer-VariableDeclaration][Rename in expressions]' +
          '[Old]',
        expressionStatement,
        this.logger,
        this.obfuscatedSourceCode,
...
class ConvertingObjectPatternPropertiesTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  deobfuscate() {
    const propertyNodes = esquery(
      this.ast,
      `ObjectPattern > Property[shorthand=false]`,
    );
    propertyNodes.forEach((propertyNode) => {
      if (this._objectPatternPropertyNodeIsCloned(propertyNode)) {
        astOperations.ASTModifier.logDebugNode(
          '[Converting][PatternProperties][Changing to shorthand]' + '[Old]',
          propertyNode.parent,
          this.logger,
          this.obfuscatedSourceCode,
          true,
        ...
        astOperations.ASTModifier.logDebugNode(
          '[Converting][PatternProperties][Changing to shorthand]' + '[New]',
          propertyNode.parent,
          this.logger,
          this.obfuscatedSourceCode,
          true,
...
class ConvertingObjectExpressionTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  _deobfuscateObjectExpressionNodeWithStringKey(objectExpressionNode) {
    ...
    for (let i = 0; i < objectExpressionNode.properties.length; i++) {
      const property = objectExpressionNode.properties[i];
      ...
      if (
        !usefulModule.StringOperations.isValidIdentifierName(literalNode.value)
      ) {
        continue;
      }

      astOperations.ASTModifier.logDebugNode(
        '[Converting][ObjectExpression][Changing to non string property]' +
          '[Old]',
        property,
        this.logger,
        this.obfuscatedSourceCode,
...
class ConvertingNumberToNumericalExpressionTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  deobfuscate() {
    do {
      ...
      const binaryExpressionNodesWithLiterals = esquery(
        this.ast,
        `BinaryExpression:matches([left.type='Literal'], ` +
      ...
      binaryExpressionNodesWithLiterals.forEach((binaryExpression) => {
        ...
        const newNode = this._createNewLiteralNode(binaryExpression);
  ...
  _createNewLiteralNode(binaryExpression) {
    this.logger.debug(
      `[EVAL][BinaryExpression] ${astOperations.ASTSourceCodeOperations.generateSourceCodeFromAST(
        binaryExpression,
      )}`,
    );
    const newLiteralValue =
      astOperations.NodeEvaller.evalNodeFromASTRepresentation(binaryExpression);
  ...
  _deobfuscateNodes(astNodes) {
    ...
    for (let i = 0; i < astNodes.length; i++) {
      ...
      memberExpressionNodes.forEach((memberExpression) => {
        ...
        if (
          !usefulModule.StringOperations.isValidIdentifierName(
            propertyNode.value,
          )
        ) {
          return;
...
class ConvertingClassFieldTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  deobfuscate() {
    const literalInsideMemberDefinitionNodes = esquery(
      this.ast,
      `MethodDefinition[computed=true] > Literal`,
    );
    literalInsideMemberDefinitionNodes.forEach((literalNode) => {
      ...
      const methodDefinitionNode = literalNode.parent;
...
class ConvertingBooleanLiteralTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  _debobfuscateTrueBooleanLiteralTransformer() {
    const allArrayExpressionNodes = esquery(
      this.ast,
      `UnaryExpression[operator='!'] > ` +
