...
class ASTModifier {
  ...
  static logDebugNode(
    ...
    if (node.range && sourceCode && !forceASTGeneration) {
      ...
      try {
        nodeSourceCode =
          ASTSourceCodeOperations.generateSourceCodeFromAST(node);
      } catch (e) {}
    }
  ...
  static removeMultipleNodesWithSameParent(
    ...
    let removedAllNodesOfInterest = false;
    estraverse.replace(parent, {
      enter: function (node) {
  ...
  static insertNodesAfter(
    ...
    for (let i = 0; i < nodesToInsert.length; i++) {
      const singleNode = nodesToInsert[i];
