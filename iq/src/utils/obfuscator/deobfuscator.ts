...
class SourceCodeDeobfuscator {
  constructor(
    ...
    argv,
    currentCallIsFromEmbeddedEval = false,
  ) {
    this.logger = logger;
  ...
  deobfuscate() {
    try {
      ...
      this.logger.error(
        `[deobfuscator.js] Error when transforming the input source code to AST.` +
  ...
  _generateASTFromObfuscatedSourceCode() {
    this.logger.info(
      '[deobfuscator.js] Generating AST from obfuscated source code.',
    );
    this.ast = astOperations.ASTSourceCodeOperations.generateASTFromSourceCode(
  ...
  _deobfuscateAST() {
    ...
    const astDeobfuscator = new ASTDeobfuscator(
      this.logger,
      this.obfuscatedSourceCode,
      this.ast,
      this.argv,
      this.currentCallIsFromEmbeddedEval,
    );
  ...
  _generateSourceCodeFromDeobfuscatedAST() {
    this.logger.info(
      '[deobfuscator.js] Generating source code from deobfuscated AST.',
    );
    this.deobfuscatedSourceCode =
...
class StageFinalizingEmbeddedEval extends stageDeobfuscator.StageDeobfuscator {
  ...
  _initiateTransformers() {
    this.transformers = {
      FinalizingEvalCallExpressionTransformer:
        FinalizingEvalCallExpressionTransformer,
  ...
  deobfuscate(currentCallIsFromEmbeddedEval = false) {
    ...
    for (const transformerName in this.transformers) {
      const transformerDeobfuscatorObject =
        this._instantiateTransformerObject(transformerName);
      this.logger.info(
        `[deobfuscator.js] ${
          i + 1
        }/${nrTransformers}) Transform '${transformerName}'.`,
      );
      try {
        this._callDeobfuscationForTransformerObject(
          transformerDeobfuscatorObject,
          transformerName,
          currentCallIsFromEmbeddedEval,
        );
      } catch (e) {
        this.logger.error(
          `[deobfuscator.js] Deobfuscation error for eval-transform '${transformerName}'. ` +
  ...
  _callDeobfuscationForTransformerObject(
    transformerDeobfuscatorObject,
    transformerName,
    currentCallIsFromEmbeddedEval,
  ) {
    transformerDeobfuscatorObject.deobfuscate(currentCallIsFromEmbeddedEval);
    transformerDeobfuscatorObject.displaySourceCodeBeforeAndAfterTransformation(
      transformerName,
    );
  }
}
...
class FinalizingEvalCallExpressionTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  _deobfuscateEvalledString(node) {
    ...
    const evalContentsDeobfuscator = new SourceCodeDeobfuscator(
      this.logger,
      evalledString,
      this.argv,
      true,
...
class FinalizingEscapeSequenceTransformer extends stageDeobfuscator.TransformerDeobfuscator {
  ...
  _nodeIsDoubleEscapedString(node) {
    return (
      ...
      usefulModule.StringOperations.strContainsOnlyDoubleEscapedChars(
        node.value,
      )
    );
  }
  ...
  _createNewUnescapedStringNode(oldNode) {
    ...
    const newNode = {
      type: 'Literal',
      value: usefulModule.StringOperations.unescapeStrWithDoubleEscapedChars(
        oldNode.value,
      ),
      parent: oldNode.parent,
    };
...
class ASTDeobfuscator {
  ...
  static DeobfuscateStageClasses = {
    ...
    ControlFlowFlattening:
      stageControlFlowFlattening.StageControlFlowFlattening,
  ...
  deobfuscate() {
    let i = 0;
    const nrStages = Object.keys(
      ASTDeobfuscator.DeobfuscateStageClasses,
    ).length;
    ...
    for (const stageName in ASTDeobfuscator.DeobfuscateStageClasses) {
      ...
      this.logger.info(
        `[deobfuscator.js] ${
          i + 1
        }/${nrStages}) Deobfuscating stage '${stageName}'.`,
      );
      console.log(
        `[deobfuscator.js] ${
          i + 1
      ...
      try {
        this._callDeobfuscationForStageObject(
          stageDeobfuscatorObject,
          stageName,
        );
      } catch (e) {
        this.logger.error(
          `[deobfuscator.js] Deobfuscation error for stage '${stageName}'. ` +
            `error = ${e}. Stack = ${e.stack}`,
        );
      }
      i++;
  ...
  _instantiateStageObject(stageName) {
    const stageDeobfuscatorClass =
      ASTDeobfuscator.DeobfuscateStageClasses[stageName];
    return new stageDeobfuscatorClass(
      this.logger,
      this.obfuscatedSourceCode,
      this.ast,
      this.argv,
